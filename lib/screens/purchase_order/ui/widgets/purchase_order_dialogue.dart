...
class PurchaseOrderAllocatedDialog extends ConsumerStatefulWidget {
  final Set<String> selectedOrderIds; 
  const PurchaseOrderAllocatedDialog({super.key, required this.selectedOrderIds});
  ...
  static Future<void> show(BuildContext context, WidgetRef ref, Set<String> selectedOrderIds) async {
    await showDialog(
      ...
      builder: (_) => Dialog(
        ...
        child: PurchaseOrderAllocatedDialog(selectedOrderIds: selectedOrderIds),
      ),
    );
  }

  @override
  ConsumerState<PurchaseOrderAllocatedDialog> createState() => _PurchaseOrderAllocatedDialogState();
}

class _PurchaseOrderAllocatedDialogState extends ConsumerState<PurchaseOrderAllocatedDialog> {
  ...
  Future<void> _submit() async {
    ...
    final payload = {
      ...
      "allocated_craftsman_code": _selectedBpCode,
    };

    final success = await ref.read(purchaseOrderListProvider.notifier).assignOrder(
        context, payload);

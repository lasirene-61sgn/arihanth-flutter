...
import '../../../buyer/ui/buyer_creation_form/buyer_creation_form.dart';
...
class CraftManCreationForm extends ConsumerStatefulWidget {
  const CraftManCreationForm({super.key});

  @override
  ConsumerState<CraftManCreationForm> createState() => _CraftManCreationFormState();
}

class _CraftManCreationFormState extends ConsumerState<CraftManCreationForm> with SingleTickerProviderStateMixin {
...
// Debounce timer to avoid excessive API calls
  ...
  final _craftmanCode = TextEditingController();
  ...
  void _addWorker() {
    if (_workerNameController.text.isNotEmpty && _workerNumberController.text.isNotEmpty) {
      setState(() {
        // Create the map as Map<String, dynamic> explicitly
        final Map<String, dynamic> newWorker = {
          'id': null, // Important for the payload logic later
          'name': _workerNameController.text,
          'number': _workerNumberController.text,
          'doc': _tempWorkerDoc,
        };

        workerDetailList.add(newWorker);

        _workerNameController.clear();
        _workerNumberController.clear();
        _tempWorkerDoc = KycDocument(); // Reset for next entry
      });
    } else {
      Toaster.showError("Please enter both name and number");
    }
  }

  void _editWorker(int index) {
    final item = workerDetailList[index];
    setState(() {
      _workerNameController.text = item['name'] ?? '';
      _workerNumberController.text = item['number'] ?? '';
      _tempWorkerDoc = item['doc'] ?? KycDocument();
      workerDetailList.removeAt(index);
    });
  }
...
// Debounced pincode change handler
  ...
  Future<void> _loadBuyerData(int id) async {
    ...
    await ref.read(craftsmanListProvider.notifier).fetchCraftsmanDetail(id);

    // Get the loaded buyer from state
    final craftsman = ref.read(craftsmanListProvider).selectedCraftsman;

    if (craftsman != null) {
      _populateFields(craftsman);
    }
  }
  void _populateFields(Craftsman buyer) {
    setState(() {
      // --- TAB 1: BASIC INFO ---
      _craftmanCode.text = buyer.craftmanCode ?? '';
      ...
      _documents['brand_img'] = KycDocument(networkUrl: buyer.brandAttachmentUrl);
      ...
      if (buyer.bankDetails.isNotEmpty) {
      ...
      if (buyer.workerDetails.isNotEmpty) {
        // Use .from to ensure the type is compatible with List<Map<String, dynamic>>
        workerDetailList = List<Map<String, dynamic>>.from(
            buyer.workerDetails.map((w) => {
              'id': w.id,
              'name': w.workerName ?? '',
              'number': w.workerNumber ?? '',
              'doc': KycDocument(networkUrl: w.workerImage),
            })
        );
      }
...
// Optimized Bank Edit
  ...
  Future<void> _validateAndSubmit() async {
    ...
    Map<String, dynamic> fields = {
      ...
      "craftman_code":_craftmanCode.text,
    ...
    for (int i = 0; i < workerDetailList.length; i++) {
      fields["worker_name[$i]"] = workerDetailList[i]['name'];
      fields["worker_number[$i]"] = workerDetailList[i]['number'];
    }
    ...
    for (int i = 0; i < workerDetailList.length; i++) {
      if (workerDetailList[i]['doc'].platformFile != null) {
        files["worker_image[$i]"] = workerDetailList[i]['doc'].platformFile;
      }
    }

    // 7. Call the notifier
    await ref.read(craftsmanListProvider.notifier).saveCraftsman(
      method:id != null ? "POST":"POST" ,
      url: id != null? "api/super-admin/craftsmen/$id": "api/super-admin/craftsmen",
      field: fields,
      files: files,
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Craftsmen Creation", style: TextStyle(color: AppColor.white, fontWeight: FontWeight.bold)),
        bottom: TabBar(
  ...
  Widget _buildPermissionsTab() {
    return ListView(
      ...
      children: [
        ...
        const Text(
          "Select the screens this craftsmen is allowed to access.",
  ...
  Widget _buildGeneralTab() {
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        _input("CraftsMen Code", _craftmanCode, isReq: true),
        ...
        _input(
          ...
          validator: (v) {
            ...
            if (!gstRegex.hasMatch(v)) {
              return "Enter valid GST No (e.g. 22ABCDE1234F1Z5)";
            }
  ...
  Widget _buildStickyFooter() {
    return SafeArea(
      child: Container(
        padding: const EdgeInsets.all(16),
        child: CustomButton(
          text: id != null ? "UPDATE" : "CRAFTSMAN CREATION",
          isLoading: ref.watch(craftsmanListProvider).isSaving,
          iconData: Icons.check_circle,
          iconRight: true,
          backgroundColor: AppColor.primary,
          textColor: AppColor.textWhite,
          onPressed: ref.watch(craftsmanListProvider).isSaving ? null : _validateAndSubmit,
        ),
      ),
    );
  }
...
// Reusable File Picker
  ...
  Widget _buildWorkerTab() {
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        const Text("Add Worker Details",
            style: TextStyle(fontWeight: FontWeight.bold, color: AppColor.textPrimary, fontSize: 16)),
        const SizedBox(height: 16),

        _input("Worker Name", _workerNameController),
        _input("Worker Mobile Number", _workerNumberController, type: TextInputType.phone),

        // Reusing your existing _uploadTile logic
        _uploadTile("Worker Photo / ID", "temp_worker", customDoc: _tempWorkerDoc),
        ...
        Align(
          ...
          child: ElevatedButton.icon(
            ...
            label: const Text("Add Worker to List"),
            style: ElevatedButton.styleFrom(
              backgroundColor: AppColor.primary,
              foregroundColor: AppColor.textWhite,
              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
        ...
        const Divider(height: 40, color: AppColor.coolLavender),

        // List of added workers
        ...workerDetailList.asMap().entries.map((entry) {
          int idx = entry.key;
          var item = entry.value;
          return _buildCardItem(
            "${item['name']} - ${item['number']}",
            item['doc'],
                () => setState(() => workerDetailList.removeAt(idx)),
                () => _editWorker(idx),
          );
        }),
 // Include the final submit button here too
      ],
    );
  }
  ...
  void _handlePickedFile(String docKey, PlatformFile? platformFile) {
    setState(() {
      ...
      if (docKey == 'temp_bank') {
        ...
        doc = _tempAadharDoc;
      }else if (docKey == 'temp_worker') { // <--- Add this
        doc = _tempWorkerDoc;
      } else {

...
import 'package:arianth/screens/my_profile/riverpod/buyer_profile_notifier.dart';
...
class _BuyerProfileScreenState extends ConsumerState<BuyerProfileScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _isInitialized = true;
    Future.microtask(() => ref.read(buyerProfileNotifierProvider.notifier).fetchProfile());
  }

  void _populateFields(BuyerProfileModel profile) {
  ...
  void dispose() {
    ...
    _tanNOController.dispose(); _msmeNoController.dispose(); _bisNoController.dispose();
    ...
    _businessNameController.dispose(); _contactNameController.dispose(); _mobileController.dispose(); _emailController.dispose();
    _landlineController.dispose(); _businessEmailController.dispose(); _referredByController.dispose(); _moreInfoController.dispose();
    _doorNoController.dispose(); _shopNoController.dispose(); _complexNameController.dispose(); _buildingNameController.dispose();
    _streetNameController.dispose(); _areaController.dispose(); _pincodeController.dispose(); _cityController.dispose();
    _stateController.dispose(); _mapLocationController.dispose(); _locationGuideController.dispose();
    super.dispose();
  }

  @override
  Widget build(BuildContext context) {
    final profileState = ref.watch(buyerProfileNotifierProvider);
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: _isEditMode 
          ...
          indicatorColor: AppColor.primary, labelColor: AppColor.white, unselectedLabelColor: AppColor.black,
          labelStyle: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold),
          tabs: const [ Tab(text: "General"), Tab(text: "Address"), Tab(text: "KYC"), Tab(text: "Bank"), Tab(text: "Images") ],
        ),
      ),
      body: profile == null 
        ...
        : !_isEditMode 
            ...
            : Form(
                ...
                child: Column(
                  children: [
                    Expanded(child: TabBarView(controller: _tabController, children: [ _buildGeneralTab(), _buildAddressTab(), _buildKycTab(), _buildBankTab(), _buildImagesTab() ])),
                    if (profile.kycStatus == "pending") _buildSubmitButton(),
                  ],
                ),
              ),
    );
  ...
  Widget _buildProfileSummary(BuyerProfileModel profile) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          Text(profile.bpCode ?? "", style: const TextStyle(color: AppColor.textSecondary, fontSize: 16)),
  ...
  Future<void> _validateAndSubmit() async {
    ...
    final success = await ref.read(buyerProfileNotifierProvider.notifier).updateProfile(fields: fields, files: files);
